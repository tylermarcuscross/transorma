...
public actor UnsubscribeWorker {
    ...
    public init(store: SharedStore, transport: any HTTPTransport = PublicHTTPSClient(), intelligence: any MailIntelligence = AppleIntelligence()) {
        self.store = store; self.transport = transport; self.intelligence = intelligence
    }
    ...
    private func authorized(_ job: UnsubscribeJob) throws -> ProtectionSettings {
        ...
        guard settings.enabled, !settings.allows(job.sender), job.kind != .web || settings.useIntelligence else { throw MailError.paused }
        return settings
    }
    ...
    private func process(_ job: UnsubscribeJob) async {
        do {
            ...
            if job.kind == .oneClick {
                let response = try await transport.send(HTTPRequest(url: url, method: "POST", body: Data("List-Unsubscribe=One-Click".utf8)))
                if (200...299).contains(response.status) {
                    try store.finish(job, status: .accepted, detail: "The sender accepted the unsubscribe request. Future delivery is not guaranteed.")
                } else if response.status == 429 || (500...599).contains(response.status) {
                    try store.finish(job, status: .failed, detail: "The sender is temporarily unavailable.", retry: true)
                } else {
                    try store.finish(job, status: .unsupported, detail: "The sender did not accept the one-click request. Redirects are not followed.")
                }
            } else {
                try await navigate(job, initialURL: url)
            }
        } catch MailError.paused {
            try? store.finish(job, status: .cancelled, detail: "Stopped by protection settings. An already-sent request cannot be recalled.")
        } catch MailError.unsafeURL {
            try? store.finish(job, status: .unsupported, detail: "The unsubscribe destination did not pass network safety checks.")
        } catch MailError.unsupportedPage {
            try? store.finish(job, status: .unsupported, detail: "This unsubscribe flow requires unsupported interactions.")
        } catch {
            // Transport errors after sending a request cannot establish whether the server acted.
            try? store.finish(job, status: .uncertain, detail: "Processing was interrupted or the response could not be verified. No automatic replay.")
        }
    }
    ...
    private func navigate(_ job: UnsubscribeJob, initialURL: URL) async throws {
        ...
        for _ in 0..<5 {
            ...
            let key = request.method + request.url.absoluteString + UnsubscribeJob.digest(String(decoding: request.body ?? Data(), as: UTF8.self))
            ...
            if (300...399).contains(response.status) {
                guard let location = response.headers["location"],
                      let next = URL(string: location, relativeTo: request.url)?.absoluteURL else { throw MailError.unsupportedPage }
            ...
            guard response.status == 200,
                  ...
                  let html = String(data: response.body, encoding: .utf8) else { throw MailError.unsupportedPage }
            ...
            guard !page.blocked, !page.actions.isEmpty,
                  ...
                  let action = page.actions.first(where: { $0.id == id }) else { throw MailError.unsupportedPage }
            _ = try authorized(job)
