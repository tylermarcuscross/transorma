...
public struct UnsubscribePage: Sendable {
    ...
    public init(html: String, baseURL: URL) throws {
        ...
        confirmsUnsubscribe = ["you have been unsubscribed", "you are now unsubscribed", "you've been unsubscribed", "successfully unsubscribed", "unsubscribe successful", "removed from our mailing list"].contains { lower.contains($0) }
        blocked = ["captcha", "sign in to", "log in to", "enter your password", "verify your identity"].contains { lower.contains($0) }
            || parsed.elements("input").contains { parsed.attribute($0, "type")?.lowercased() == "password" }
            || parsed.elements("iframe").contains { (parsed.attribute($0, "src") ?? "").lowercased().contains("captcha") }
        guard !blocked else { actions = []; return }
        ...
        for node in parsed.elements("a").prefix(100) {
            ...
            guard Self.isUnsubscribeLabel(label), let href = parsed.attribute(node, "href"),
                  ...
                  (try? URLPolicy.validate(url, sameHost: baseURL.host)) != nil else { continue }
        ...
        for form in parsed.elements("form").prefix(10) {
            guard let action = parsed.attribute(form, "action"),
                  ...
                  [nil, "application/x-www-form-urlencoded"].contains(parsed.attribute(form, "enctype")?.lowercased()) else { continue }
            let controls = parsed.descendants(form).filter { ["input", "button", "select", "textarea"].contains(parsed.name($0)) }
            ...
            for control in controls {
                ...
                let type = (parsed.attribute(control, "type") ?? (parsed.name(control) == "button" ? "submit" : "text")).lowercased()
                if type == "hidden", parsed.name(control) == "input", let name, name.utf8.count < 256, value.utf8.count < 8192 {
            ...
            guard !unsafe, buttons.count == 1, let button = buttons.first, Self.isUnsubscribeLabel(button.0) else { continue }
        ...
        actions = candidates.prefix(12).enumerated().map { PageAction(id: $0.offset, label: String($0.element.0.prefix(200)), request: $0.element.1) }
    }
    ...
    public static func isUnsubscribeLabel(_ label: String) -> Bool {
        ...
        guard lower.count <= 200,
              !["resubscribe", "do not", "don't", "cancel", "delete", "account", "paid", "purchase", "billing", "payment", "keep receiving", "stay subscribed"].contains(where: lower.contains) else { return false }
    ...
    public static func emailLinks(in html: String) -> [URL] {
        ...
        return parsed.elements("a").prefix(300).compactMap { node in
            guard isUnsubscribeLabel(parsed.text(node)), let href = parsed.attribute(node, "href"),
                  let url = URL(string: href), (try? URLPolicy.validate(url)) != nil else { return nil }
            return url
        }
    }
...
private final class HTMLDocument {
    ...
    init(_ html: String) throws {
        ...
        guard let document = html.withCString({ htmlReadMemory($0, Int32(html.utf8.count), nil, "UTF-8", options) }) else { throw MailError.unsupportedPage }
    ...
    func text(_ root: xmlNodePtr?) -> String {
        ...
        func walk(_ node: xmlNodePtr, depth: Int) {
            ...
            if node.pointee.type == XML_TEXT_NODE, let content = node.pointee.content {
                ...
                pieces.append(value); remaining -= value.count
            }
            var child = node.pointee.children
            while let node = child { walk(node, depth: depth + 1); child = node.pointee.next }
        }
        walk(root, depth: 0)
    ...
    func descendants(_ root: xmlNodePtr) -> [xmlNodePtr] {
        ...
        func walk(_ parent: xmlNodePtr, depth: Int) {
            ...
            while let node = child, result.count < 10_000 {
                result.append(node); walk(node, depth: depth + 1); child = node.pointee.next
            }
        }
        walk(root, depth: 0)
        return result
