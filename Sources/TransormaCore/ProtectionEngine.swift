...
public enum MarketingPolicy {
    public static func isCandidate(subject: String, text: String, hasUnsubscribe: Bool) -> Bool {
        ...
        let protected = ["receipt", "invoice", "order confirmation", "order #", "order number", "your order", "your purchase",
            "payment", "statement", "verification", "verify your", "security", "password", "sign-in", "sign in", "login",
            "one-time", "authentication", "shipping", "delivery update", "tracking number", "appointment", "reservation",
        ...
        guard !subject.hasPrefix("re:"), !subject.hasPrefix("fwd:"), !subject.hasPrefix("fw:"),
              !protected.contains(where: { subject.contains($0) || text.contains($0) }) else { return false }
        // A list header alone includes many important newsletters and account notices.
        let promotion = ["sale", "discount", "% off", "shop now", "special offer", "limited time", "save today", "coupon", "promo code", "new arrivals", "clearance"]
...
public actor ProtectionEngine {
    ...
    public init(store: SharedStore, verifier: DKIMVerifier = DKIMVerifier(), intelligence: any MailIntelligence = AppleIntelligence()) {
        self.store = store; self.verifier = verifier; self.intelligence = intelligence
    }
    ...
    public func assess(raw: Data) async -> MailAssessment {
        ...
        do {
            ...
            guard let sender = message.sender, !settings.allows(sender),
                  message.single("auto-submitted") == nil || message.single("auto-submitted")?.lowercased() == "no" else { return .keep }
            ...
            guard MarketingPolicy.isCandidate(subject: subject, text: text, hasUnsubscribe: !candidates.isEmpty) else { return .keep }
            ...
            if let url = message.oneClickURL, verified.covers("list-unsubscribe"), verified.covers("list-unsubscribe-post") {
                ...
                guard settings.useIntelligence, intelligence.available, candidates.count == 1, let url = candidates.first else { return .keep }
...
enum HeaderText {
    static func decode(_ text: String) -> String {
        ...
        for match in regex.matches(in: text, range: NSRange(text.startIndex..., in: text)).reversed() {
            ...
            let data = mode == "b" ? Data(base64Encoded: raw) : MIMEParser.quotedPrintable(Data(raw.replacingOccurrences(of: "_", with: " ").utf8))
            let encoding: String.Encoding = charset == "iso-8859-1" ? .isoLatin1 : .utf8
            if let data, let decoded = String(data: data, encoding: encoding), let range = Range(match.range, in: result) { result.replaceSubrange(range, with: decoded) }
        }
        return result
    }
}
