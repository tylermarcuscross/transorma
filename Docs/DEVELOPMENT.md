...
`App/Resources/AppIcon.icon` is the editable Icon Composer source used by the Dock and Finder. It contains a graphite background and two SVG layers forming a large white T with a vermilion paper fold. The neutral base takes its cues from black-and-white photographs of industrial mail sorting; the fold adds the same red accent as the menu bar. Xcode compiles that same source into both the companion app and Mail extension, including the native Liquid Glass appearances and fallback icon resources. Open the `.icon` bundle in Xcode's **Open Developer Tool → Icon Composer** to adjust its layers.
