...
3. In Mail → Settings → Extensions, confirm Transorma's T appears in both the list and details panel. Quit and reopen Mail if it still displays cached puzzle-piece icons after an update. Enable the extension and allow full message content access. Confirm the app shows a recent Mail heartbeat.
...
For the CLI route, add the approved Apple Account to Xcode → Settings → Accounts and ensure `DEVELOPMENT_TEAM` in `Config/Shared.xcconfig` matches the enrolled team. Run `make build-signed`; Xcode manages development certificates, destination-device registration, and provisioning profiles. Quit the Development preview, install `.build/Xcode/Build/Products/Debug/Transorma.app` in Applications, and open that copy. Quit and reopen Mail so it loads the installed extension. Confirm its registered path with `pluginkit -m -A -D -v -i me.tylercross.transorma.mailextension` and check for a new Mail heartbeat in Settings. A checkbox selected on an old Development extension does not establish that live processing is running.

When investigating a missed message, first check the running app and registered extension configurations, shared activation state, and last Mail heartbeat. Then use the original raw message to check unsubscribe headers, full-body signature verification, classification, and the decision deadline. A screenshot or Mail's own unsubscribe button does not establish that Transorma supports that message's unsubscribe route. Keep original messages and recipient tokens out of Git; use synthetic, signed fixtures for committed regression tests. MailKit does not replay an already-downloaded message just because a working extension was installed afterward.

For login launch, use the signed app installed in Applications and a fresh test user's preferences. On the first launch with usable shared storage, verify registration appears under System Settings → General → Login Items & Extensions. Log out and back in to confirm Transorma launches and can resume its queue. Turn login launch off in Transorma, relaunch, and verify it stays off. Repeat with access revoked in System Settings: the app must show the approval action without re-registering itself. Registration failures must leave the toggle reflecting macOS's actual state and show a retryable error. Automated tests use a fake service; actual registration and login launch remain signed installation checks. Starting Transorma does not launch Mail or allow processing while the Mac sleeps.
...
- Review the privacy manifests and App Store privacy answers against final behavior, including optional PCC and unsubscribe websites. Both manifests declare no tracking or developer-collected data. The app declares UserDefaults reason `CA92.1` for its private login setup preference; the extension declares no required-reason APIs. Revisit if storage, diagnostics, analytics, or APIs change.
