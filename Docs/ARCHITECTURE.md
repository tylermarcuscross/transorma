...
`MailDocument` accepts network CRLF and uniformly LF-delimited local message sources. It restores LF to CRLF at the byte level before parsing, MIME decoding, and DKIM canonicalization, as required by the network-format assumption in [RFC 6376 §3.4](https://www.rfc-editor.org/rfc/rfc6376.html#section-3.4). Other octets remain unchanged. Mixed line endings and bare carriage returns are rejected, and the normalized message still has to meet the size limits. Signature verification and signed-header coverage remain mandatory for unsubscribe candidates.

Message eligibility requires an unsubscribe route and preserves replies and recognized transactional content. When on-device intelligence is enabled and available, eligible messages pass DKIM verification before the model decides whether they are marketing; matching sale keywords is unnecessary. A privacy-policy footer alone does not prevent this assessment, while policy topics in the subject still preserve the message. With intelligence off or unavailable, the stricter English keyword rules apply, including the policy-text exclusions. A rejected, uncertain, or failed model assessment preserves the message.
...
Preview mode is explicit in the model and visible in both sections, including the activation label and status. Preview settings cannot indicate actual Mail protection. The shared scheme and `make run` retain this mode; `make build-signed` explicitly builds Debug with the account's provisioning assets for installation.
