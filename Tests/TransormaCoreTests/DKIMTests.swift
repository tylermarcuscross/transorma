...
struct SignedFixture {
    ...
    init(subject: String = "Summer sale: 40% off", body: String = "Save today. Shop now for our limited time sale.",
         oneClick: Bool = true, algorithm: String = "ed25519-sha256", canonicalization: String = "relaxed/relaxed") throws {
        var fields = ["From: Store <offers@store.example.com>", "To: recipient@example.net", "Subject: \(subject)", "Content-Type: text/html; charset=utf-8"]
        if oneClick {
            fields += ["List-Unsubscribe: <https://store.example.com/unsubscribe?token=recipient>", "List-Unsubscribe-Post: List-Unsubscribe=One-Click"]
        }
        ...
        let bodyHash = Data(SHA256.hash(data: DKIMVerifier.canonicalBody(rawBody, relaxed: relaxedBody))).base64EncodedString()
        let names = fields.map { $0.components(separatedBy: ":")[0].lowercased() }.joined(separator: ":")
        let dkim = "DKIM-Signature: v=1; a=\(algorithm); c=\(canonicalization); d=store.example.com; s=test; h=\(names); bh=\(bodyHash); b="
        var signed = Data()
        for field in fields { signed += DKIMVerifier.canonicalHeader(Data(field.utf8), relaxed: relaxedHeader) + Data([13, 10]) }
        ...
        if algorithm == "ed25519-sha256" {
            ...
            signature = try #require(SecKeyCreateSignature(key, .rsaSignatureMessagePKCS1v15SHA256, signed as CFData, nil)) as Data
        }
        raw = Data((([dkim + signature.base64EncodedString()] + fields).joined(separator: "\r\n") + "\r\n\r\n").utf8) + rawBody
    }
}

@Test(arguments: ["rsa-sha256", "ed25519-sha256"], ["simple/simple", "relaxed/relaxed", "relaxed/simple", "simple/relaxed"])
...
func rejectsTampering(change: String) async throws {
    ...
    let modified: String = switch change {
    ...
    await #expect(throws: (any Error).self) { try await DKIMVerifier(resolver: fixture.dns).verify(MailDocument(raw: Data(modified.utf8))) }
}

@Test func ignoresForgedAuthenticationResults() async throws {
    let message = try MailDocument(raw: Data("From: offers@store.example.com\r\nSubject: Sale\r\nAuthentication-Results: mx.example; dkim=pass\r\n\r\nShop now".utf8))
...
@Test func canonicalizationMatchesPublishedRules() {
    #expect(DKIMVerifier.canonicalHeader(Data("SUBJect :  A\t B\r\n C  ".utf8), relaxed: true) == Data("subject :A B C".utf8))
    #expect(DKIMVerifier.canonicalBody(Data(" C \t\r\nD \t E\r\n\r\n".utf8), relaxed: true) == Data(" C\r\nD E\r\n".utf8))
