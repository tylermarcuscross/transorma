...
2. Build with the shared Transorma scheme and install the signed app into Applications.
3. Enable the extension in Mail and allow full message content access. Confirm the app shows a recent Mail heartbeat.
4. Use a dedicated test mailbox and sender you control. Confirm new signed promotions request Trash, transactional messages stay, pause takes effect, and the keep list cancels pending work. No actual email was sent or changed as part of this repository implementation.
...
6. Verify extension memory, inference latency, and Mail callback deadlines under large arrival bursts. The code bounds parsing and callback time; sustained-load behavior is not yet measured. A callback timeout leaves mail in place. An unsubscribe already queued immediately before the timeout can still complete.
